...
public class HitICommand extends ListenerAdapter implements ICommand {

    @Override
    public File getPath() {
        return new File("src/main/resources/hit.gif");
    }
    ...
    public void onMessageReceived(MessageReceivedEvent e) {
        ...
        if (message.getContentRaw().startsWith(getCommand())) {
            ...
            for (val member : message.getMentionedMembers()) {
                channel.sendMessage(Objects.requireNonNull(e.getMember()).getAsMention() + " ударил(-а) " + member.getAsMention()).queue();
